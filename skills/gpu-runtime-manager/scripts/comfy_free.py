...
def main() -> int:
    parser = argparse.ArgumentParser(description="Wait for ComfyUI idle, then release its VRAM")
    ...
    try:
        ...
        with manager.locked(): data = manager.release(GPUOwner.VIDEO, args.dry_run)
        if args.json: print(json.dumps({"ok": True, **data}, indent=2))
        elif args.dry_run:
            for action in data["would"]: print(f"would {action}")
        else: print("ComfyUI memory released")
        return 0
